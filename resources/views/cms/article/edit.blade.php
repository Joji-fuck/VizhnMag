...
<div class="max-w-4xl bg-white mx-auto p-8 rounded shadow">
    <h1 class="text-2xl font-bold mb-6">Редактировать статью "{{$article->title}}"</h1>
    ...
    <form action="{{ route('cms.article.update', $article->id) }}" method="POST" enctype="multipart/form-data">
        @csrf

        <!-- Заголовок -->
        <div class="mb-4">
            <label for="title" class="block text-gray-700 font-bold mb-2">Заголовок статьи</label>
            <input type="text" name="title" id="title" class="w-full border rounded px-3 py-2 outline-none focus:border-blue-500" value="{{ $article->title }}" placeholder="Введите заголовок...">
        </div>

        <!-- Фото обложки -->
        <div class="mb-4">
            <label for="image" class="block text-gray-700 font-bold mb-2">Обложка новости</label>
            <img class="border-solid border-inherit" src="{{asset('storage/' . $article->image)}}" alt="{{$article->title}}" width="250" height="60"/>
        ...
        <div class="mb-6">
            ...
            <textarea name="content" id="content-editor">{{ old('content', $article->content)}}</textarea>
        </div>
